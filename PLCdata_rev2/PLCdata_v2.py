...
frame4 = Frame(win,height=85, bg='light green')
...
# this is your headers for excel sheet only populates if new sheet started
  ...
  file2 = "C:\\Users\\yourpathhere\\documents\\test_" + current_datetime + ".xlsx" #creates file with datetime
  try:
...
def stoplogging():
    global readok
    readok = False  
    readlinelbl.config(text="STOPPED LOGGING") 
#-----------------------------------------------------------------------------------------------------------------#
...
#watchtaglbl = Label(frame2, text='Watching Tags', fg='blue')
...
#scrollbar = Scrollbar(frame2, orient=VERTICAL, command=text_box_gettags.yview,  background='lightblue', activebackground='blue')
#scrollbar.grid(row=0, column=1, sticky='ns')  # Attach scrollbar next to the Text widget

# Connect the Text widget and Scrollbar
#text_box_gettags.configure(yscrollcommand=scrollbar.set)
# GET ALL plc tags
...
readlinelbl = Label(frame4, text='Data Read', font='arial')
